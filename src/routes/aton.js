...
router.post("/add", funciones.isAuthenticated, funciones.hasSanPrivileges, async (req, res) => {
    ...
    try {
        ...
        if (existe.length == 0) {
            ...
            const dir = join(__dirname, '../public/img/imagenes/', newBalizamiento.nif);
...
router.post("/editCaracteristicas/:nif", funciones.isAuthenticated, funciones.hasSanPrivileges, async (req, res) => {
    ...
    const newBaliza = { ...req.body };

    // Asegurar tipos
    if (newBaliza.periodo) newBaliza.periodo = parseInt(newBaliza.periodo);
    if (newBaliza.categoria) newBaliza.categoria = parseInt(newBaliza.categoria);
    ...
    try {
        ...
        if (nifViejo !== newBaliza.nif) {
            ...
            // 3️⃣ Renombrar carpeta de fotos si existe
            const oldDir = join(__dirname, '../public/img/imagenes/', nifViejo);
            const newDir = join(__dirname, '../public/img/imagenes/', newBaliza.nif);
            try {
                await fs.rename(oldDir, newDir);
            } catch (err) {
                if (err.code === 'ENOENT') {
                    console.warn("La carpeta de fotos no existía:", oldDir);
                } else {
                    throw err;
                }
            }
        ...
        // 4️⃣ Log más limpio
        funciones.insertarLog(req.user.usuario, "UPDATE balizamiento", JSON.stringify(newBaliza));

        req.flash("success", "Baliza modificada correctamente");
        res.redirect("/aton/plantilla/" + newBaliza.nif);

    } catch (error) {
        console.error("Error editando baliza:", error);
        req.flash("error", `Hubo un error al modificar el AtoN ${newBaliza.nif}: ${error.message}`);
